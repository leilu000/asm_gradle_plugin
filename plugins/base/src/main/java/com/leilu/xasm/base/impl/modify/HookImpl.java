...
public class HookImpl implements IHook {
    ...
    public byte[] startHook(byte[] sourceData) {
        byte[] result = hook(sourceData, cn -> hookAnnotation(cn.methods));
        return hook(result, cn -> hookMethod(cn.methods));
    }
    ...
    private void hookAnnotation(List<MethodNode> methodNodes) {
        for (MethodNode mn : methodNodes) {
            ...
            realHook(mn, new OnHookMethodListener() {
                @Override
                public InsnList onMethodStart(MethodNode mn, MethodInfo methodInfo) {
                    return data.listener.onMethodStart(mn, methodInfo);
                }

                @Override
                public boolean visitInsnNode(AbstractInsnNode abstractInsnNode, MethodInfo methodInfo) {
                    return data.listener.visitInsnMode(abstractInsnNode, methodInfo);
                }

                @Override
                public InsnList onMethodEnd(MethodNode mn, MethodInfo methodInfo) {
                    return data.listener.onMethodEnd(mn, methodInfo);
                }
            });
        }
    }

    private void hookMethod(List<MethodNode> methodNodes) {
        ...
        for (MethodNode mn : methodNodes) {
            ...
            realHook(mn, new HookMethodListener(data));
        }
        // 再查找是否有匹配的lambda表达式的方法，有则hook
        hookLambdaMethod(methodNodes);
    }

    private void hookLambdaMethod(List<MethodNode> methodNodes) {
    ...
    private static class HookMethodListener implements OnHookMethodListener {
        ...
        public HookMethodListener(HookMethodData data) {
            this.mData = data;
        }

        @Override
        public InsnList onMethodStart(MethodNode mn, MethodInfo methodInfo) {
            return mData.listener.onMethodStart(mn, methodInfo);
        }

        @Override
        public boolean visitInsnNode(AbstractInsnNode abstractInsnNode, MethodInfo methodInfo) {
            return mData.listener.visitInsnNode(abstractInsnNode, methodInfo);
        }

        @Override
        public InsnList onMethodEnd(MethodNode mn, MethodInfo methodInfo) {
            return mData.listener.onMethodEnd(mn, methodInfo);
        }
    }
    ...
    private void realHook(MethodNode mn, OnHookMethodListener listener) {
        ...
        InsnList insertStartList = listener.onMethodStart(mn, methodInfo);
        // 在方法开始之前插入
        ...
        while (listIterator.hasNext()) {
            ...
            boolean needRemove = listener.visitInsnNode(abstractInsnNode, methodInfo);
            ...
            if (ASMUtil.isMethodEnd(abstractInsnNode)) {
                InsnList insertEndList = listener.onMethodEnd(mn, methodInfo);
